...
The Silver layer adheres to 3 principles:

* **Governance mode storage:** Because silver unstructured data must support recomputable pipelines and services updates, the silver bucket operates in “Governance Mode” with object versioning (rather than the strict immutable “Compliance Mode” locks of Bronze). This protects against accidental deletion while allowing authorized Airflow processes to overwrite silver data assets during pipeline replays (e.g., re-validation or software updates).
...
* **Unstructured pipeline:** Runs automated integrity checks, verifies file patient IDs against the Master Patient Index (MPI), and adds this connection to an **Identify bridge table**. Files are also lossless compressed.
...
For structured clinical data (EDC), raw payloads land in the locked Bronze object store, triggering Airflow DAGs to ingest, parse, transform, and validate the structured data and send it to the EHRbase API (the openEHR Clinical Data Repository). EHRbase functions as our raw clinical data control hub.

The aim of this process is to convert raw data into OpenEHR archetypes so that we adhere to the following requirements:

- long term persistence
- support a wide range of applications
- is “easy” to maintain
- holds semantic meaning for downstream serving.

OpenEHR is the natural fit for the initial ingestion of clinical data, but we could complement it with FHIR (external tools integration) or OMOP (analytics). Due to the nature of the institute, the most logical combination would be OpenEHR for initial ingestion and data model standardisation (Silver layer), and OMOP for research and analytics (Gold layer).

**In other words, OpenEHR is used for persisting data with complete granularity, whereas OMOP introduces a less granular canonical data model(s) that can be more easily served and customized to downstream analytics or processing.**

#### Advantages of the openEHR two-level approach

* **Long term persistence and zero schema migrations:** The OpenEHR reference model provides a stable, universal data structure that never changes based on clinical workflows.
* **Granular context preservation** and rich clinical vocabulary
* **Clinical context and engineering separation:** Software engineers manage the core platform infrastructure (EHRBase), while clinical experts define the clinical semantics. This clearly divides technical software development from medical knowledge management (archetypes and templates).
...
- **OMOP is optimized for analytics**, meaning mapping into the CDM is inherently lossy for complex clinical sources.
...
* **Concept mapping overhead:** OMOP relies on standard vocabularies. New or trial-specific metrics need concept resolution and CDM design before they are cleanly analytics-ready. This is not necessarily slower at capture, but requires more upfront mapping work and coordination if OMOP is the only persistence layer.
...
* **Analytical overhead:** openEHR structures are deeply nested and hierarchical. Heavy aggregations or analyses via AQL introduce computational overhead and require specialised openEHR knowledge.
* **Serving mismatch:** Cohort builders and ML tooling generally expect flat tabular CDMs (e.g. OMOP), not nested data, pushing **ad-hoc** flattening to downstream consumers (repetitive work, different baseline data, etc).
...
For payload transformation we will use Pydantic for two use cases: transformation and validation. While EHRbase already enforces strict schema constraints at ingestion, it is likely that some EDC records may contain errors or missing data. Therefore, before payloads are sent to the EHRbase API, a process uses Pydantic to:
...
The engineering overhead shifts from database management to precise clinical data modeling. The platform minimizes custom design by drawing from a mature and international ecosystem:
...
Because omics, imaging, and multimedia data are heavily bound to underlying research methodologies, monolithic standardization is not feasible. The bronze-to-silver unstructured data DAG (built incrementatlly) focuses on:
...
* FASTQ Integrity: Run gzip -t <file.fastq> to check file integrity
* BAM/CRAM sanity check: samtools quickcheck.

##### Multimedia & Bio-imaging data

For DICOM assets, the validation layer focuses strictly on structural metadata and heuristics rather than deep image analysis. The validation process uses standard DICOM libraries (e.g., [dicom-validator](https://pypi.org/project/dicom-validator/)) to verify mandatory header tags:
...
* Technical Attributes: Reference genome build used (e.g., HG38), alignment software version, sequencing instrument model (e.g., Illumina NovaSeq 6000), etc.
...
All extracted metadata attributes and data lineage are published to a dedicated Silver Metadata Catalogue (e.g., publishing it as well to [OpenMetadata](https://open-metadata.org/)).
...
To optimize I/O throughput and storage costs without sacrificing biological fidelity, the platform prioritizes interoperable, widely supported lossless compression formats over pure compression ratios:

Below you can find a non-exhaustive list of standards for compression per data type:
...
After processing and storing these files, we also store (in the identity bridge table) the link between each file and the respective patient information:
...
*This will be important later on for modality linking, the knowledge graph and GDRP compliance.*
